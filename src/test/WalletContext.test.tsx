...
const TestComponent: React.FC = () => {
  const { walletInfo, isConnecting, error, connectMetaMask, disconnectWallet, getDisplayAddress } =
    useWallet();

  return (
    <div>
      <div data-testid="wallet-address">{walletInfo.address}</div>
      <div data-testid="wallet-status">{walletInfo.isConnected ? 'Connected' : 'Disconnected'}</div>
...
describe('WalletContext', () => {
  ...
  it('should connect to wallet when connectMetaMask is called', async () => {
    ...
    expect(screen.getByTestId('wallet-display-address').textContent).toBe('0x1234...7890');
  });
