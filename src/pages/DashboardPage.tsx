...
export const DashboardPage: React.FC<DashboardPageProps> = ({
    ...
    // Use real wallet functionality if enabled
    const walletContext = useWallet();

    // Determine which wallet state to use
    const isConnected = useRealWallet ? walletContext.walletInfo.isConnected : walletConnected;
    ...
    const handleDisconnectWallet = () => {
        if (useRealWallet) {
            ...
            onConnectWallet(); // Toggle mock wallet state
        }
    };
    return (
                    ...
                    <div className="bg-white/90 backdrop-blur-sm p-6 rounded-xl shadow-xl border border-white/20 hover:shadow-2xl transition-all duration-300">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p
                                    className={`text-sm font-semibold ${(useRealWallet ? walletContext.walletInfo.isConnected : walletConnected)
                                            ? 'text-emerald-600' : 'text-red-600'}`
                                    }
                                >
                                    {(useRealWallet ? walletContext.walletInfo.isConnected : walletConnected)
                                        ? 'Connected' : 'Disconnected'}
                                </p>
                            </div>
                            <Wallet
                                className={`w-8 h-8 ${(useRealWallet ? walletContext.walletInfo.isConnected : walletConnected)
                            ...
                            {(useRealWallet ? walletContext.walletInfo.isConnected : walletConnected) ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="text-sm text-gray-600 break-all bg-gray-50 p-3 rounded-lg">
                                        {useRealWallet
                                            ? walletContext.walletInfo.address
                                            : '0x742d35cc6634C0532925a3b8D6aD8a7e15b2a9d1'}
                                    </div>
                                    <button
                                        onClick={useRealWallet ? walletContext.disconnectWallet : onConnectWallet}
                                ...
                                <div className="space-y-4">
                                    <div className="text-center py-6">
                                        ...
                                        <button
                                            onClick={useRealWallet ? walletContext.connectMetaMask : onConnectWallet}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="p-4 bg-purple-50/80 backdrop-blur-sm rounded-lg border border-purple-100/50">
                                    ...
                                    <p className="text-sm text-purple-700">
                                        {mockMarketData.topLocations[0].name} -{' '}
                                        {mockMarketData.topLocations[0].avgPrice} avg
                                    </p>
                                </div>
                            </div>
                        </div>
